# GO OVER LOGIC ONCE MORE!


def find_next_empty(puzzle):
    # finds the next position on the puzzle (row, col) that is blank. Represent it with -1
    # return position (row, col) tuple (or (None, None) if there is none)
...
def is_valid(puzzle, guess, row, col):
    ...
    # now the column
    # col_vals = []
    # You are parsing through the each row and storing the values in a particular column
    col_vals = [puzzle[i][col] for i in range(9)]
    if guess in col_vals:
        return False

    # and then the square
    row_start = (row // 3) * 3 # 10 // 3 = 3, 5 // 3 = 1, 1 // 3 = 0. A clever math trick! It starts you are 0, 3, or 6 index in a row or col
    col_start = (col // 3) * 3
