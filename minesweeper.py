...
class Board:
    def __init__(self, dim_size, num_bombs):
        ...
        # get the board
        self.board = self.make_new_board() # Sets up the board with required number of bombs
        self.assign_values_to_board()      # Assigns value to each position on the board corresponding to number of neighbouring bombs
    ...
    def get_num_neighboring_bombs(self, row, col):
        ...
        for r in range(max(0, row-1), min(self.dim_size-1, row+1)+1):     # Max/Min statements ensure we stay in bounds
            for c in range(max(0, col-1), min(self.dim_size-1, col+1)+1): # The 3x3 area at and around position is iterated over
                if r == row and c == col:
    ...
    def dig(self, row, col):
        ...
        # return True if successful dig, False if bomb dug --> BOOLEAN
        ...
        if self.board[row][col] == '*': # ---> GAME OVER!
            return False
        elif self.board[row][col] > 0:  # Location neighboring bomb(s)
            return True
...
def play(dim_size=10, num_bombs=10):
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        ...
        # Rejex Split Explained: re.split(',(\\s)*', input...) takes the string input, splits it at the comma (',)
        # ingores any spaces (\\s) the user may have inputted, and repeat this (*) until done

        # Set value for row, col
        row, col = int(user_input[0]), int(user_input[-1]) 
        # re.split sometimes has fluff in between the list, [0] and [-1] takes the first and last element of the list (which is what we care about)
        ...
        if not safe: # <--> if safe != True
            break    # It is GAME OVER!
